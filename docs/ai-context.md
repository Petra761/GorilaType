...
- **Npgsql 7+**: `Trust Server Certificate` está eliminado y lanza error; usar `SSL Mode=Require;`.
...
- Conexión directa a Supabase (IPv6) → NO. Usar Session Pooler IPv4 (`aws-0-us-east-1.pooler.supabase.com:5432`).
- Una sola connection string → NO. Dos connection strings (`DefaultConnection` con rol `gorilatype_app` para runtime y `MigrationsConnection` con rol `postgres` para DDL/migraciones).
- Migraciones con usuario de aplicación → NO. Migraciones con `postgres` vía `--connection "<MigrationsConnection>"`.
- RLS deshabilitado o bypass en runtime → NO. RLS activo en tablas de dominio, scoped vía `AppDbContext.BeginUserScopedTransactionAsync`.
...
6. Commits siempre en inglés, en formato `tipo: descripción`, sin scope entre paréntesis y sin punto final.
